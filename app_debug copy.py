...
def retry_topic_batch(
    ...
    aggression_level = "FINAL WARNING"
...
YOUR OUTPUT:"""
    ...
    try:
        progress(0.95, desc=f"Retry #{retry_attempt}/{MAX_RETRIES} - Extracting Topics...")
...
def normalize_topics_to_pillars_v12(df, 
    ...
    for idx, campaign in enumerate(df[campaign_col].unique(), 1):
        ...
        progress(progress_val, desc=f"[STEP 4/5] Normalizing campaign {idx}/{total_campaigns}")
...
def process_file(
    ...
    try:
        ...
        # ============================
        # STEP 1/5: PILLAR + TOPIC + SENTIMENT
        # ============================
        if generate_topic or generate_sentiment:
            logging.info("\n" + "="*80)
            logging.info("[STEP 1/5] PILLAR + TOPIC + SENTIMENT (MASTER ROWS, ELIGIBLE CONTENT)")
            logging.info("="*80)
            ...
            if len(df_to_process) > 0:
                ...
                for batch_num in range(total_batches):
                    ...
                    progress_val = 0.1 + (batch_num / total_batches) * 0.25
                    progress(progress_val, desc=f"[STEP 1/5] Processing {batch_num + 1}/{total_batches}")
                ...
                if generate_topic:
                    ...
                    logging.info(f"[STEP 1/5] ✅ Pillar: {pillar_success}/{len(df)} ({pillar_success/len(df)*100:.1f}%)")
        ...
        # ============================
        # STEP 2/5: SPOKESPERSON
        # ============================
        if generate_spokesperson and mainstream_count > 0:
            logging.info("\n" + "="*80)
            logging.info("[STEP 2/5] SPOKESPERSON (MAINSTREAM MASTER ROWS, ELIGIBLE CONTENT)")
            logging.info("="*80)
            ...
            if len(df_mainstream) > 0:
                ...
                for batch_num in range(total_batches):
                    ...
                    progress_val = 0.40 + (batch_num / total_batches) * 0.10
                    progress(progress_val, desc=f"[STEP 2/5] Spokesperson {batch_num + 1}/{total_batches}")
                ...
                logging.info(f"[STEP 2/5] ✅ Spokesperson: {success_count}/{mainstream_count} ({success_count/mainstream_count*100:.1f}%)")
            
            df.loc[social_mask, 'New Spokesperson'] = ''
        
        # ============================
        # STEP 3/5: RETRY FOR EMPTY TOPICS
        # ============================
        if generate_topic:
            logging.info("\n" + "="*80)
            logging.info(f"[STEP 3/5] RETRY FOR EMPTY TOPIC/PILLAR (MAX {MAX_RETRIES}x)")
            logging.info("="*80)
            ...
            for retry_attempt in range(1, MAX_RETRIES + 1):
                ...
                logging.info(f"[STEP 3/5] Retry #{retry_attempt}: {empty_count} empty topic/pillar rows")

                if empty_count == 0:
                    logging.info("[STEP 3/5] ✅ All topics/pillars filled, stopping retry early")
                    break
                ...
                for batch_num in range(total_batches):
                    ...
                    progress_val = 0.50 + (batch_num / total_batches) * 0.10
                    progress(progress_val, desc=f"[STEP 3/5] Retry {batch_num + 1}/{total_batches}")
                ...
                for idx in df_retried.index:
                    ...
                    if 'Topic' in df_retried.columns:
                        retried_topic = df_retried.at[idx, 'Topic']
                        if retried_topic and str(retried_topic).strip():
                            df.at[idx, 'Topic'] = retried_topic
            ...
            pillar_filled_after_retry = df['Pillar'].notna() & (df['Pillar'].astype(str).str.strip() != '')
            topic_filled_after_retry = df['Topic'].notna() & (df['Topic'].astype(str).str.strip() != '')
            
            pillar_success_retry = pillar_filled_after_retry.sum()
            topic_success_retry = topic_filled_after_retry.sum()
            
            tracker.add_step_stat("Pillar (after retry)", pillar_success_retry, len(df))
            tracker.add_step_stat("Topic (after retry)", topic_success_retry, len(df))
            
            logging.info(f"[STEP 3/5] ✅ After Retry - Pillar: {pillar_success_retry}/{len(df)} ({pillar_success_retry/len(df)*100:.1f}%)")
            logging.info(f"[STEP 3/5] ✅ After Retry - Topic: {topic_success_retry}/{len(df)} ({topic_success_retry/len(df)*100:.1f}%)")
        ...
        if generate_topic:
            logging.info("\n" + "="*80)
            logging.info("[STEP 4/5] NORMALIZATION (TOPICS → PILLARS PER CAMPAIGN)")
            logging.info("="*80)
            
            progress(0.60, desc="[STEP 4/5] Normalizing Topics → Pillars (per campaign)...")
            ...
            if 'Campaigns' not in df.columns:
                ...
                pillar_filled_normalized = df['Pillar'].notna() & (df['Pillar'].astype(str).str.strip() != '')
                pillar_success_normalized = pillar_filled_normalized.sum()
                tracker.add_step_stat("Pillar (normalized)", pillar_success_normalized, len(df))
                ...
                logging.info(f"[STEP 4/5] ✅ Pillar (normalized): {pillar_success_normalized}/{len(df)} ({pillar_success_normalized/len(df)*100:.1f}%)")
                logging.info(f"[STEP 4/5] 📊 Grouping: {unique_topics_count} topics → {unique_pillars_final} pillars ({grouping_rate:.1f}% reduction)")
        
        # ============================
        # STEP 5/5: SYNCHRONIZE (FALLBACK)
        # ============================
        if generate_topic:
            logging.info("\n" + "="*80)
            logging.info("[STEP 5/5] SYNCHRONIZE TOPIC & PILLAR (FALLBACK)")
            logging.info("="*80)

            progress(0.80, desc="[STEP 5/5] Synchronizing Topic ↔ Pillar...")

            # Phase A: Topic kosong, Pillar ada → Topic = Pillar
            mask_topic_empty = (
                (df['Topic'].isna() | (df['Topic'].astype(str).str.strip() == '') | df['Topic'].apply(lambda x: is_invalid_value(str(x)))) &
            ...
            # Phase B: Pillar kosong, Topic ada → Pillar = Topic
            mask_pillar_empty = (
                (df['Pillar'].isna() | (df['Pillar'].astype(str).str.strip() == '') | df['Pillar'].apply(lambda x: is_invalid_value(str(x)))) &
            ...
            logging.info(
                f"[STEP 5/5] ✅ Synchronize - Topic←Pillar: {topic_filled_from_pillar}, "
                f"Pillar←Topic: {pillar_filled_from_topic}"
            )

            # Final stats
            pillar_final = df['Pillar'].notna() & (df['Pillar'].astype(str).str.strip() != '')
            topic_final = df['Topic'].notna() & (df['Topic'].astype(str).str.strip() != '')
            
            pillar_success_final = pillar_final.sum()
            topic_success_final = topic_final.sum()
            
            tracker.add_step_stat("Pillar (final)", pillar_success_final, len(df))
            tracker.add_step_stat("Topic (final)", topic_success_final, len(df))
            
            logging.info(f"[STEP 5/5] ✅ Final - Pillar: {pillar_success_final}/{len(df)} ({pillar_success_final/len(df)*100:.1f}%)")
            logging.info(f"[STEP 5/5] ✅ Final - Topic: {topic_success_final}/{len(df)} ({topic_success_final/len(df)*100:.1f}%)")
        
        # ============================
        # SPOKESPERSON NORMALIZATION
        # ============================
        if generate_spokesperson:
            progress(0.85, desc="Normalizing Spokesperson...")
        ...
        with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
            ...
            meta_data = [
                ...
                {"key": "version", "value": "v12.1 - Opsi 2: Retry → Normalize → Sync"},
        ...
        logging.info("\n" + "="*80)
        logging.info("✅ PROCESSING COMPLETE - v12.1 (Opsi 2)")
        logging.info("="*80)
...
def create_gradio_interface():
    with gr.Blocks(title="Insights Generator v12.1", theme=gr.themes.Soft()) as app:
        gr.Markdown("# 📊 Insights Generator v12.1 - Optimized Workflow")
        gr.Markdown("**NEW:** Step 1: Extract → Step 2: Spokesperson → Step 3: Retry → Step 4: Normalize → Step 5: Sync (Fallback)")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                gen_topic = gr.Checkbox(label="📌 Pillar & Topic (all channels) - Enhanced with retry", value=False)
        ...
        def process_wrapper(file_path, sheet_name, language, topic, sentiment, spokesperson, conf, progress=gr.Progress()):
            try:
                if not file_path:
                    return None, "", "❌ Please upload an Excel file", gr.Button(interactive=True)
                
                if not sheet_name:
                    return None, "", "❌ Please select a sheet", gr.Button(interactive=True)
                
                if not any([topic, sentiment, spokesperson]):
                    return None, "", "❌ Please select at least one feature", gr.Button(interactive=True)
                ...
                if error:
                    return None, "", error, gr.Button(interactive=True)
                
                if not result_path:
                    return None, "", "❌ Processing failed", gr.Button(interactive=True)
                ...
                return result_path, stats_str, "✅ Processing completed!", gr.Button(interactive=True)
                
            except Exception as e:
                error_msg = f"❌ Error: {str(e)}"
                import traceback
                traceback.print_exc()
                return None, "", error_msg, gr.Button(interactive=True)
        ...
        process_btn.click(
            ...
            outputs=[output_file, stats_output, error_output, process_btn]
        )
